...
from teleop_sdk.providers import LeaderTeleopProvider, ZeroResidualPolicy
...
def main(args: argparse.Namespace) -> None:
    ...
    output_dir = (
        ...
        else (PROJECT_ROOT / "logs" / ("assisted_teleop_dataset" if args.checkpoint is not None else "teleop_dataset")).resolve()
    )
    ...
    try:
                ...
                rollout.meta.update(
                    {
                        ...
                        "viewer": False,
                        "collection_mode": "assisted_teleop" if args.checkpoint is not None else "teleop_only",
        ...
        with mujoco.viewer.launch_passive(env.model, env.data, key_callback=key_callback) as viewer:
            ...
            pending_reason = ""
            copilot_enabled = args.checkpoint is not None
            ...
            if args.checkpoint is not None:
                print(f"[INFO] Copilot is {'ON' if copilot_enabled else 'OFF'}")
                ...
                if mode == "RECORDING":
                    ...
                    if args.checkpoint is None:
                        ...
                        if teleop_active:
                            ...
                            if copilot_enabled:
                                ...
                                residual_status = "copilot_active"
                            else:
                    ...
                    transitions.append(
                        Transition(
                            ...
                            base_joint_target=np.asarray(step_info["target_joint_pos"], dtype=np.float32).copy(),
                            base_gripper_target=np.asarray([step_info["target_gripper"]], dtype=np.float32),
                            teleop_active=bool(teleop_active),
                            success=bool(step_info.get("success", False)),
                            alpha=float(step_info.get("alpha", assist_alpha)),
                            copilot_enabled=bool(args.checkpoint is not None and copilot_enabled),
                            ...
                            policy_latency_ms=float(policy_latency_ms if args.checkpoint is not None else 0.0),
                        )
                    )
